...
# type hierarchy (Phase 1 of the full rewrite — see
# .claude/plans/structured-wibbling-wigderson.md for the complete design).
#
# Built from scratch (no GeometryBasics, no StaticArrays) but preserving
# exactly the arithmetic/indexing interface the rest of the package
# already relies on: `p[i]` indexing, `+`/`-`/unary `-`, scalar `*`/`/`,
# `dot`/`norm`/`normalize`, and destructuring iteration (`x, y = p`). This
# is what lets every later phase migrate type *names* without touching
# formula *bodies*.
#
...
# still return EGPoint, exactly like today's conflated Point2. No
# CGAL-style strict affine-space rules.
# -------------------------------------------------------------------------

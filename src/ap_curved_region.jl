# -------------------------------------------------------------------------
# APCircularSector2, APCircularSegment2, APAnnularSector2, APInterstice2,
# APCurvilinearTriangle2, APCurvilinearQuadrilateral2, APCurvilinearNgon2 —
# all <: APPolygon, all sharing the generic `sides`-based area/perimeter
# from ap_polygon.jl. Each just implements `sides(p)` (a mix of
# APSegment/APCircularArc2) and its own rotate/reflection/homothety
# (pointwise per side).
#
# `APInterstice2` here is buildable directly from 3 arcs; the
# `interstices(c1, c2, c3)` convenience constructor (given 3 mutually
# tangent circles) lives in ap_apollonius.jl instead, since it needs
# `tangent_circles` (the CCC Apollonius solver).
# -------------------------------------------------------------------------

# `_side_p1`/`_side_p2`/`_side_length`/`_side_greens_term`/`_side_scale`
# for an APCircularArc2 side (the APSegment ones are in ap_polygon.jl).
...
# under a general `APAffineMap`, so a region built from circular arcs can
# come back with elliptic ones — see ap_transform.jl). `_side_length` is
...
struct APAnnularSector2{T<:Real} <: APPolygon{2,T}
    ...
    # auto-generated default outer constructor `APAnnularSector2(outer::APCircularArc2{T},
...
# --- APCurvilinearQuadrilateral2 (new) --------------------------------------
...
# --- APCurvilinearNgon2 (n mixed sides, general case) -----------------------
...
# `convert(APSide{T}, s)` doesn't work directly: `APSide{T}` is a `Union`,
...
# `convert(::Type{APSegment{2,T}}, ...)`/etc. that already exists for it.
...
# A plain `[seg, arc, seg]` array literal infers as `Vector{APCurve{2,T}}`
# (the common abstract supertype), which doesn't convert to
# `Vector{APSide{T}}` (Julia containers are invariant) — so accept any
...
# `APSide{T}[...]` literal to convert them itself), for the same

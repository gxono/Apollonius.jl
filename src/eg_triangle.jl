...
# EGEllipse2/EGParabola2. Formula bodies are unchanged from the
# Point2-based originals; every named triangle center, axis, circle,
# derived triangle and inellipse/circumellipse/hyperbola/parabola in the
# original file is ported here, in the same order.
#
# `_other_two(i::Integer)` is already untyped in triangle.jl and works for
# free (no point type in its signature at all).
# -------------------------------------------------------------------------
...
on those 5 points, rather than reproducing tkz-elements' own longer
axis-construction algorithm — both give the same curve, since a conic is
uniquely determined by 5 points and this one is already known to pass
through all 5.
"""

...
translate(ang::EGAngle2, v::EGVector) =
    EGAngle2(translate(ang.vertex, v), translate(ang.a, v), translate(ang.b, v))

"""
    distance(p::EGPoint, ang::EGAngle2; mode::Symbol=:region)

Distance from `p` to the wedge `ang` (see [`EGAngle2`](@ref)). The
...
"""
function distance(p::EGPoint, ang::EGAngle2; mode::Symbol=:region)
    _check_distance_mode(mode)
    d = min(distance(p, EGRay(ang.vertex, ang.a)), distance(p, EGRay(ang.vertex, ang.b)))
    mode == :boundary && return d
    return p in ang ? zero(d) : d
end
distance(ang::EGAngle2, p::EGPoint; mode::Symbol=:region) = distance(p, ang; mode=mode)
...
function EGHalfPlane2(boundary::EGLine{2}, p::EGPoint{2})
...
translate(hp::EGHalfPlane2, v::EGVector) = EGHalfPlane2(translate(hp.boundary, v), hp.side)
...
"""
    distance(p::EGPoint, hp::EGHalfPlane2; mode::Symbol=:region)
...
inside.
"""
function distance(p::EGPoint, hp::EGHalfPlane2; mode::Symbol=:region)
    _check_distance_mode(mode)
...
function EGStrip2(line1::EGLine{2,T1}, line2::EGLine{2,T2}; atol=1e-9) where {T1,T2}
    ...
    return EGStrip2{T}(convert(EGLine{2,T}, line1), convert(EGLine{2,T}, line2); atol=atol)
end
...
translate(s::EGStrip2, v::EGVector) = EGStrip2(translate(s.line1, v), translate(s.line2, v))

"""
    distance(p::EGPoint, s::EGStrip2; mode::Symbol=:region)
...
"""
function distance(p::EGPoint, s::EGStrip2; mode::Symbol=:region)
    _check_distance_mode(mode)

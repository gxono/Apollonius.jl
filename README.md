...
Every exported type is its own struct, prefixed `AP` (`APPoint`,
`APCircle2`, `APTriangle`, ...) and organized into a real abstract type
...
and every other closed shape genuinely *is* an `APPolygon`, so
`area`/`perimeter`/`centroid`/`is_convex`/`point_in_polygon` are defined
**once**, generically, on `APPolygon` itself — not reimplemented per type.
...
- **Types**: `APPoint`, `APVector`, `APSegment`, `APLine` (infinite),
  `APRay`, `APCircle2`, `APTriangle`, `APQuadrilateral`, `APStraightNgon`,
  `APBoundingBox`, `APAngle2`, `APHalfPlane2`, `APStrip2`,
  `APCircularArc2`, `APEllipticArc2`, `APParabolicArc2`,
  `APHyperbolicArc2`, `APCircularSector2`, `APCircularSegment2`,
  `APAnnularSector2`, `APInterstice2`, `APCurvilinearTriangle2`,
  `APCurvilinearQuadrilateral2`, `APCurvilinearNgon2`, `APEllipse2`,
  `APParabola2`, `APHyperbola2`, `APAffineMap`.
- **Vector algebra**: `APVector` is a separate type from `APPoint` (a free
  direction/displacement rather than a location), with `norm`, `dot`,
  `normalize`, `angle_between`, `angle_at`. Arithmetic between the two
  stays deliberately permissive — `APPoint - APPoint` still gives back an
  `APPoint`, not a vector — so existing point-arithmetic formulas (like a
...
- **Constructions**: `midpoint`, `distance`, `projection`, `reflection`,
  ...
  every type in the package — `APPoint`, `APSegment`, `APLine`, `APRay`,
  every conic (`APCircle2`, `APEllipse2`, `APParabola2`, `APHyperbola2`) and
  conic arc (`APCircularArc2` and the rest), the whole `APPolygon` family
  (straight and curvilinear — `APTriangle`, `APStraightNgon`,
  `APQuadrilateral`, `APCircularSector2`, `APInterstice2`, ...), and the
  unbounded sets `APAngle2`/`APHalfPlane2`/`APStrip2` — except
  `APBoundingBox`, which only supports the subset of transforms
  (`translate`, uniform scaling) that keep it axis-aligned, since
  `rotate`/`reflection` generally wouldn't. `APVector` gets `rotate` and
...
- **Batch-transform macros**: `@translate`/`@rotate`/`@homothety`/
  ...
  block pattern folded into one `APBoundingBox` via `bbox_union` instead
  of transforming each shape.
- **Intersections**: `intersection` between any pair of `APLine`/`APCircle2`,
  between finite `APSegment`s, and between an `APLine` and any conic
  (`APEllipse2`/`APParabola2`/`APHyperbola2`).
...
- **Pole/polar duality**: `polar_line`/`pole` with respect to a circle
  (also `polar_line` for `APEllipse2`/`APParabola2`/`APHyperbola2`, which
  `tangent_points`/`tangent_lines` for those already use internally).
- **The polygon family** (`APPolygon`): `area`, `perimeter`, `centroid`,
  `is_convex`, `point_in_polygon`, `convex_hull` — defined once,
  generically, via a Green's-theorem walk over each shape's `sides`, so
  the same formulas cover straight-sided shapes (`APTriangle`,
  `APQuadrilateral`, `APStraightNgon`) and curved-sided regions
  (`APCircularSector2`, `APCircularSegment2`, `APAnnularSector2`,
  `APInterstice2`, the `APCurvilinear*` family) alike.
- **Quadrilaterals**: `APQuadrilateral` (a dedicated 4-vertex type,
  distinct from a general `APStraightNgon`) — `sides`, `diagonals`,
  `diagonal_intersection`, `centroid` (plain vertex average, unlike the
  generic area-weighted one), `area`, `perimeter`, `is_convex`,
  `is_cyclic`, `APBoundingBox`.
- **Bounding boxes**: `APBoundingBox` can be built from any bounded shape
  in the package — `APSegment`, every conic (`APCircle2`/`APEllipse2`) and
  every conic arc (bounded even when the full curve isn't, like a
  hyperbolic or parabolic arc), and the whole `APPolygon` family (straight
  and curvilinear alike). Left out on purpose: `APLine`/`APRay`
  (infinite), `APHyperbola2`/`APParabola2` as full curves (also infinite),
  and the unbounded sets `APHalfPlane2`/`APStrip2`/`APAngle2`. Also:
...
- **Angles as objects and regions**: `APAngle2(vertex, a, b)` keeps the
  three defining points around instead of just a number — `measure`
  (signed), `normalized_measure`, `abs` (unsigned), `is_direct`, and (since
  it's a genuine `APSet`) `p in ang` for the infinite wedge it sweeps.
- **Unbounded regions**: `APHalfPlane2` (a boundary line plus a side),
  `APStrip2` (the band between two parallel lines) — both support `p in s`,
  and the same `rotate`/`reflection`/`homothety` as everything else.
- **Circular (and conic) arcs, sectors and segments**: `APCircularArc2`
  ...
  `APEllipticArc2`/`APParabolicArc2`/`APHyperbolicArc2` for the other
  conics) — `measure`, `arc_length`, `point_on_arc`, `midpoint`.
  `APCircularSector2` (the "pie slice"), `APCircularSegment2` (the "cap")
  and `APAnnularSector2` (the "ring slice", between two concentric arcs)
  each wrap an arc — `area`/`perimeter` come for free from the generic
  `APPolygon` machinery, plus `p in shape`.
- **Interstices**: `interstices` — the curvilinear-triangle gap(s) between
  ...
  `APInterstice2`s bounded by three `APCircularArc2`s.
- **Curvilinear polygons**: `APCurvilinearTriangle2`/
  `APCurvilinearQuadrilateral2`/`APCurvilinearNgon2` — closed regions with
  any mix of straight (`APSegment`) and curved (`APCircularArc2`) sides,
...
- **Inversion**: `inversion` (points), `invert` (lines/circles to their
  image circle; an `APSegment` to an `APSegment` or `APCircularArc2`; an
  `APTriangle`/`APStraightNgon` to an `APCurvilinearNgon2`, since straight
...
- **Affine transformations**: `APAffineMap` (composable with `∘`), callable
  ...
  a region built from circular arcs (`APCircularSector2`, `APAnnularSector2`,
  `APInterstice2`, ...) comes back as the more general
...
- **Conics**: `APEllipse2`, `APParabola2` and `APHyperbola2` — point-on-curve
  ...
  `foci`/`asymptotes` (hyperbola), `intersection` with an `APLine`, and
  `tangent_points`/`tangent_lines` from an external point (via the polar
  line of the point, intersected with the conic). `APEllipse2` and
  `APHyperbola2` also have a bifocal constructor:
  `APEllipse2(f1, f2, a_or_point)` / `APHyperbola2(f1, f2, a_or_point)`.
...
- **`p in shape`** (`Base.in`) works for point-membership tests across the
  board: every `APPolygon` (`APTriangle`, `APQuadrilateral`,
  `APStraightNgon`, `APCircularSector2`, `APCircularSegment2`,
  `APAnnularSector2`, and the rest) gets it for free from the one generic
  `point_in_polygon`, and `APBoundingBox`, `APEllipse2`, `APParabola2`
  (region on the focus side), `APHyperbola2` (on or beyond either branch),
  `APAngle2` (its infinite wedge), `APHalfPlane2` and `APStrip2` each
  define their own.

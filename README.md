...
- **Constructions**: `midpoint`, `distance`, `projection`, `reflection`,
  `rotate`, `homothety`, `barycenter`, `parallel_through`,
  `perpendicular_through`, `perpendicular_bisector`, `angle_bisectors`,
  `angle_trisectors`, `golden_ratio_point`, `harmonic_conjugate`,
  `apollonius_circle`. `reflection`/`rotate`/`homothety` work on every type
  in the package — `EGPoint`, `EGSegment`, `EGLine`, `EGRay`, `EGCircle2`,
  `EGTriangle`, `EGStraightNgon`, `EGQuadrilateral`, `EGAngle2`,
  `EGCircularArc2` (and every other conic arc), `EGInterstice2`,
  `EGEllipse2`/`EGParabola2`/`EGHyperbola2` — except `EGBoundingBox`, which
  only supports the subset of transforms (translation, uniform scaling)
  that keep it axis-aligned.
...
- **Bounding boxes**: `bbox_width`/`bbox_height`, `bbox_center`,
  `bbox_diagonal`, `bboxes_intersect`, `bbox_intersection`, and
  `p in bbox` (`Base.in`) for point membership.
...
- **Affine transformations**: `EGAffineMap` (composable with `∘`, callable
  on `EGPoint`/`EGVector`/`EGSegment`/`EGLine`/`EGRay`/`EGTriangle`/
  `EGStraightNgon`/`EGQuadrilateral`/`EGAngle2`, and on `EGCircle2` —
  returning an `EGEllipse2`, since a general affine map doesn't send a
  circle to another circle), `affine_map` (from 3 point correspondences),

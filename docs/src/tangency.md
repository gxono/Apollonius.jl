...
* [`offset_line`](@ref) shifts a line by a signed distance — the tool these
  constructions use internally to turn "tangent to a line at distance `r`"
  into "passes through" the offset line, and occasionally useful directly.
* [`tangent_circles_with_radius`](@ref) fixes the radius of the solution in
  advance and asks for circles of exactly that radius tangent to two given
  lines or a line and a circle.
